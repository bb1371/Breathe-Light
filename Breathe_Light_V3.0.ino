/*  
    ...
    When a person is feeling anxious and wants stress relief, they press a button on the Breathe Light to begin this code.
    ...
    will be 3 NeoPixels that are lit up in Blue. These colors/thresholds are given to the subject via a sticker on the device so that they
    know what their heart rate is at. 

    After the heart rate is detected, it goes through an anxiety exercise which aids in deep, slow inhaling and exhaling. At the beginning of this
    exercise, all four LEDs are green. As the exercise goes on, the LEDs turn blue one by one which corresponds to when the subject should be inhaling. 
    After all four LEDs are lit, the subject is to hold their breath, and as the LEDs turn back to green they can slowly exhale. This is repeated three times.

    After the anxiety exercise is performed, the code will detect and display the persons heart rate again to determine if they were able to calm down.
...
const int PulsePin = 12; // Initialize the pin that the PulseSensor is connected to on the Flora
...
void loop() {
  ...
  if (buttonState != lastButtonState){ // If the state of the button has changed (i.e if the button was pressed)
    colorWipe(strip.Color(50, 0, 0, 0), 50); // Initialize a red colorwipe
    ...
    // Set up the anxietyExercise() function by setting all LEDs to Green.
    ...
    anxietyExercise(); // Repeat a second time
    anxietyExercise(); // Repeat a third time
...
void anxietyExercise() {
  ...
  // Begin Inhaling color pattern (change the LEDs to blue light, one by one)
  // When the first LED lights up, begin to inhale, and when all 4 are lit up, stop inhaling
  ...
  // Begin Exhaling color pattern (Change LED's back to green, one by one)
  
  strip.setPixelColor(3, 0, 50, 0, 0); // (Pin, Red, Green, Blue, White)
  strip.show();
  delay (1000);
...
void heartRateDisplay(){
  ...
  strip.setPixelColor(0, 50, 50, 50, 50);
  strip.setPixelColor(1, 50, 50, 50, 50);
  strip.setPixelColor(2, 50, 50, 50, 50);
  strip.setPixelColor(3, 50, 50, 50, 50);
  ...
  // Initialize good (green) Heart Rate zone (0-70)
...
// Initialize the bad (> 90 BPM) Heart rate zone (Red)
